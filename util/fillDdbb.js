...
async function createRecipe(data) {
	...
	await session
		...
		if(undefined !== recipeId) {
			await createNodes(recipeId, data.dietLabels, 'dl', 'DietLabels', 'r:DIET_LABELS');
			await createNodes(recipeId, data.healthLabels, 'hl', 'HealthLabels', 'r:HEALTH_LABELS');
			await createNodes(recipeId, data.ingredientLines, 'il', 'IngredientLines', 'r:INGREDIENT_LINES');
			await createAuxiliarNodes(recipeId, data.totalNutrients, 'tn', 'TotalNutrients', 'r:TOTAL_NUTRIENTS');
		}
}
...
async function createAuxiliarNodes(recipeId, data, nodeFirst, nodeSecond, relation) {
	for (let key in data) {		
		await session
			.run(
				`MATCH (recipe:Recipe)
